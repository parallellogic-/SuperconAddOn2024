 ...
 337                     ; 136       result = (uint16_t) ((InputClockFrequencyMHz * 1000000) / (OutputClockFrequencyHz * 3));
 ...
 359  007e               L331:
 360                     ; 141       result = (uint16_t) ((InputClockFrequencyMHz * 1000000) / (OutputClockFrequencyHz * 25));
 ...
 414  00b8               L131:
 415                     ; 164     result = (uint16_t)((InputClockFrequencyMHz * 1000000) / (OutputClockFrequencyHz << (uint8_t)1));
 ...
 436                     ; 167     if (result < (uint16_t)0x0004)
 438  00d3 a30004        	cpw	x,#4
 439  00d6 2403          	jruge	L341
 440                     ; 170       result = (uint16_t)0x0004;
 442  00d8 ae0004        	ldw	x,#4
 444  00db               L341:
 445  00db 1f08          	ldw	(OFST-1,sp),x
 446                     ; 176     I2C->TRISER = (uint8_t)(InputClockFrequencyMHz + (uint8_t)1);
 448  00dd 7b15          	ld	a,(OFST+12,sp)
 449  00df 4c            	inc	a
 450  00e0               L141:
 451  00e0 c7521d        	ld	21021,a
 452                     ; 181   I2C->CCRL = (uint8_t)result;
 454  00e3 7b09          	ld	a,(OFST+0,sp)
 455  00e5 c7521b        	ld	21019,a
 456                     ; 182   I2C->CCRH = (uint8_t)((uint8_t)((uint8_t)(result >> 8) & I2C_CCRH_CCR) | tmpccrh);
 ...
 470                     ; 191   I2C->OARL = (uint8_t)(OwnAddress);
 472  00fa 7b11          	ld	a,(OFST+8,sp)
 473  00fc c75213        	ld	21011,a
 474                     ; 192   I2C->OARH = (uint8_t)((uint8_t)(AddMode | I2C_OARH_ADDCONF) |
 474                     ; 193                    (uint8_t)((OwnAddress & (uint16_t)0x0300) >> (uint8_t)7));
 ...
 578  0132 81            	ret	
 579  0133               L371:
 580                     ; 215     I2C->CR1 &= (uint8_t)(~I2C_CR1_PE);
 ...
 638  013f 81            	ret	
 639  0140               L512:
 640                     ; 238     I2C->CR1 &= (uint8_t)(~I2C_CR1_ENGC);
 ...
 698  014c 81            	ret	
 699  014d               L732:
 700                     ; 263     I2C->CR2 &= (uint8_t)(~I2C_CR2_START);
 ...
 758  0159 81            	ret	
 759  015a               L162:
 760                     ; 286     I2C->CR2 &= (uint8_t)(~I2C_CR2_STOP);
 ...
 819  0166 81            	ret	
 820  0167               L303:
 821                     ; 309     I2C->CR2 &= (uint8_t)(~I2C_CR2_SWRST);
 ...
 880  0173 81            	ret	
 881  0174               L523:
 882                     ; 334     I2C->CR1 |= I2C_CR1_NOSTRETCH;
...
1113                     ; 408   assert_param(IS_I2C_DUTYCYCLE_OK(I2C_DutyCycle));
1115                     ; 410   if (I2C_DutyCycle == I2C_DUTYCYCLE_16_9)
1117  01a8 a140          	cp	a,#64
1118  01aa 2605          	jrne	L534
1119                     ; 413     I2C->CCRH |= I2C_CCRH_DUTY;
...
1129                     ; 420 }
1132  01b5 81            	ret	
1155                     ; 427 uint8_t I2C_ReceiveData(void)
1155                     ; 428 {
1156                     	switch	.text
1157  01b6               _I2C_ReceiveData:
1161                     ; 430   return ((uint8_t)I2C->DR);
1163  01b6 c65216        	ld	a,21014
1166  01b9 81            	ret	
1231                     ; 440 void I2C_Send7bitAddress(uint8_t Address, I2C_Direction_TypeDef Direction)
1231                     ; 441 {
1232                     	switch	.text
1233  01ba               _I2C_Send7bitAddress:
1235  01ba 89            	pushw	x
1236       00000000      OFST:	set	0
1239                     ; 443   assert_param(IS_I2C_ADDRESS_OK(Address));
1241                     ; 444   assert_param(IS_I2C_DIRECTION_OK(Direction));
1243                     ; 447   Address &= (uint8_t)0xFE;
1245  01bb 7b01          	ld	a,(OFST+1,sp)
1246  01bd a4fe          	and	a,#254
1247  01bf 6b01          	ld	(OFST+1,sp),a
1248                     ; 450   I2C->DR = (uint8_t)(Address | (uint8_t)Direction);
...
1299                     ; 462 }
1302  01cb 81            	ret	
1526                     ; 578 ErrorStatus I2C_CheckEvent(I2C_Event_TypeDef I2C_Event)
1526                     ; 579 {
1527                     	switch	.text
...
1535                     ; 580   __IO uint16_t lastevent = 0x00;
1537  01cf 5f            	clrw	x
1538  01d0 1f04          	ldw	(OFST-2,sp),x
1540                     ; 581   uint8_t flag1 = 0x00 ;
1542                     ; 582   uint8_t flag2 = 0x00;
1544                     ; 583   ErrorStatus status = ERROR;
1546                     ; 586   assert_param(IS_I2C_EVENT_OK(I2C_Event));
1548                     ; 588   if (I2C_Event == I2C_EVENT_SLAVE_ACK_FAILURE)
1550  01d2 1e07          	ldw	x,(OFST+1,sp)
1551  01d4 a30004        	cpw	x,#4
1552  01d7 2609          	jrne	L136
1553                     ; 590     lastevent = I2C->SR2 & I2C_SR2_AF;
...
1589                     ; 599   if (((uint16_t)lastevent & (uint16_t)I2C_Event) == (uint16_t)I2C_Event)
...
1717                     ; 648   return (I2C_Event_TypeDef)lastevent;
1719  0237 1e05          	ldw	x,(OFST-1,sp)
1722  0239 5b06          	addw	sp,#6
1723  023b 81            	ret	
1938                     ; 679 FlagStatus I2C_GetFlagStatus(I2C_Flag_TypeDef I2C_Flag)
1938                     ; 680 {
1939                     	switch	.text
...
1947                     ; 681   uint8_t tempreg = 0;
1949  023e 0f02          	clr	(OFST+0,sp)
1951                     ; 682   uint8_t regindex = 0;
1953                     ; 683   FlagStatus bitstatus = RESET;
1955                     ; 686   assert_param(IS_I2C_FLAG_OK(I2C_Flag));
1957                     ; 689   regindex = (uint8_t)((uint16_t)I2C_Flag >> 8);
1959  0240 9e            	ld	a,xh
1960  0241 6b01          	ld	(OFST-1,sp),a
1962                     ; 691   switch (regindex)
1965                     ; 708     default:
1965                     ; 709       break;
1966  0243 4a            	dec	a
...
1996                     ; 709       break;
1998  025d               L3101:
1999                     ; 713   if ((tempreg & (uint8_t)I2C_Flag ) != 0)
2001  025d 7b04          	ld	a,(OFST+2,sp)
2002  025f 1502          	bcp	a,(OFST+0,sp)
2003  0261 2704          	jreq	L5101
2004                     ; 716     bitstatus = SET;
2006  0263 a601          	ld	a,#1
2009  0265 2001          	jra	L7101
2010  0267               L5101:
2011                     ; 721     bitstatus = RESET;
2013  0267 4f            	clr	a
2015  0268               L7101:
2016                     ; 724   return bitstatus;
2020  0268 5b04          	addw	sp,#4
2021  026a 81            	ret	
2065                     ; 759 void I2C_ClearFlag(I2C_Flag_TypeDef I2C_FLAG)
2065                     ; 760 {
2066                     	switch	.text
2067  026b               _I2C_ClearFlag:
2069  026b 89            	pushw	x
2070       00000002      OFST:	set	2
2073                     ; 761   uint16_t flagpos = 0;
2075                     ; 763   assert_param(IS_I2C_CLEAR_FLAG_OK(I2C_FLAG));
2077                     ; 766   flagpos = (uint16_t)I2C_FLAG & FLAG_Mask;
2079  026c 01            	rrwa	x,a
2080  026d 5f            	clrw	x
2081  026e 02            	rlwa	x,a
2082  026f 1f01          	ldw	(OFST-1,sp),x
2084                     ; 768   I2C->SR2 = (uint8_t)((uint16_t)(~flagpos));
...
2092  0277 85            	popw	x
2093  0278 81            	ret	
2259                     ; 791 ITStatus I2C_GetITStatus(I2C_ITPendingBit_TypeDef I2C_ITPendingBit)
2259                     ; 792 {
2260                     	switch	.text
...
2268                     ; 793   ITStatus bitstatus = RESET;
2270                     ; 794   __IO uint8_t enablestatus = 0;
2272  027c 0f03          	clr	(OFST-1,sp)
2274                     ; 795   uint16_t tempregister = 0;
2276                     ; 798     assert_param(IS_I2C_ITPENDINGBIT_OK(I2C_ITPendingBit));
2278                     ; 800   tempregister = (uint8_t)( ((uint16_t)((uint16_t)I2C_ITPendingBit & ITEN_Mask)) >> 8);
2280  027e 9e            	ld	a,xh
2281  027f a407          	and	a,#7
2282  0281 5f            	clrw	x
2283  0282 97            	ld	xl,a
2284  0283 1f01          	ldw	(OFST-3,sp),x
2286                     ; 803   enablestatus = (uint8_t)(I2C->ITR & ( uint8_t)tempregister);
2288  0285 c6521a        	ld	a,21018
2289  0288 1402          	and	a,(OFST-2,sp)
2290  028a 6b03          	ld	(OFST-1,sp),a
2292                     ; 805   if ((uint16_t)((uint16_t)I2C_ITPendingBit & REGISTER_Mask) == REGISTER_SR1_Index)
...
2301                     ; 808     if (((I2C->SR1 & (uint8_t)I2C_ITPendingBit) != RESET) && enablestatus)
...
2309                     ; 811       bitstatus = SET;
2311  02a3 200b          	jp	LC005
2312                     ; 816       bitstatus = RESET;
2313  02a5               L1311:
2314                     ; 822     if (((I2C->SR2 & (uint8_t)I2C_ITPendingBit) != RESET) && enablestatus)
...
2322                     ; 825       bitstatus = SET;
2324  02b0               LC005:
2326  02b0 a601          	ld	a,#1
2329  02b2 2001          	jra	L7311
2330  02b4               L1411:
2331                     ; 830       bitstatus = RESET;
2334  02b4 4f            	clr	a
2336  02b5               L7311:
2337                     ; 834   return  bitstatus;
2341  02b5 5b06          	addw	sp,#6
2342  02b7 81            	ret	
2387                     ; 871 void I2C_ClearITPendingBit(I2C_ITPendingBit_TypeDef I2C_ITPendingBit)
2387                     ; 872 {
2388                     	switch	.text
2389  02b8               _I2C_ClearITPendingBit:
2391  02b8 89            	pushw	x
2392       00000002      OFST:	set	2
2395                     ; 873   uint16_t flagpos = 0;
2397                     ; 876   assert_param(IS_I2C_CLEAR_ITPENDINGBIT_OK(I2C_ITPendingBit));
2399                     ; 879   flagpos = (uint16_t)I2C_ITPendingBit & FLAG_Mask;
2401  02b9 01            	rrwa	x,a
2402  02ba 5f            	clrw	x
2403  02bb 02            	rlwa	x,a
2404  02bc 1f01          	ldw	(OFST-1,sp),x
2406                     ; 882   I2C->SR2 = (uint8_t)((uint16_t)~flagpos);
